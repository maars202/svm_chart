...
def generate_data(n_samples, dataset, noise):
    if dataset == "moons":
        ...
        combinedd = pd.concat([features, targets], axis = 'columns')
        combinedd.columns = ['Feature_1', "Feature_2", "Feature_3", "Feature_4", "targets"]
        # only binary classfication allowed in this api as multiclass needs different confusion matrix calculation
        combinedd = combinedd[combinedd["targets"].isin([0,1])]
        X, y = combinedd.drop(columns = ["targets"])[['Feature_1', 'Feature_2']], combinedd["targets"]
